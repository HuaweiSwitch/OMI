...
#if defined(CONFIG_POSIX)
static int  s_ignoreAuthCalls = 1;
#endif

extern struct nc_session *netconf_session;

#define IP_LENGTH_MAX 64 
char host[IP_LENGTH_MAX];
...
void  SetHostIp(char* managentip)
{
    memcpy(host, managentip, IP_LENGTH_MAX);
}
...
Printf("***test:user is %s, password is %s\n", user, password);
...
/*test netconf*/
{
    ...
    if(netconf_session == NULL)
    {
        ...
        Printf("***test:Host IP is %s\n", host);
        
        netconf_session = nc_session_connect(host, port, user, password, cpblts);        
    }
    
    if (netconf_session == NULL) 
    {
       // exit(1);
      Printf("***test:Eorror!netconf_session is NULL***\n", netconf_session);
