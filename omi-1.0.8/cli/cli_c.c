    ...
    const MI_Char* argv[])
{
    ...
    for (;;)
    {
        ...
        else if (Tcsncmp(state.opt, PAL_T("--"), 2) == 0)
        {
#if defined(CONFIG_ENABLE_WCHAR)
            char tmp[PAL_MAX_PATH_SIZE];

            StrWcslcpy(tmp, state.opt+2, sizeof tmp);
            if (IsNickname(tmp))
            {
                char tmp2[PAL_MAX_PATH_SIZE];

                StrWcslcpy(tmp2, state.arg, sizeof tmp);
                if (SetPathFromNickname(tmp, tmp2) != 0)
                    err(PAL_T("SetPathFromNickname() failed"));
            }
#else
            if (IsNickname(state.opt+2))
            {
...
void MI_MAIN_CALL SignalHandler(int signo)
{
    ...
    if (gop.ft)
    {
        ...
        /* Close opeartion will be hung due to */
